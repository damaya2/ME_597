...
AU_TO_MS2 = 0.00273  # Canonical accel to m/s^2
...
def set_3d_equal_aspect(ax, x_data, y_data, z_data, padding_km=2000.0):
    ...
    without altering relative object scale.
    """
    max_range = (
        np.array(
            [
    ...
    # Force true 1:1:1 geometric proportions in 3D
...
def animate_trajectory_3d(
    ...
    # Render Celestial Bodies (Physical radii stay proportional)
    ...
    def update(frame):
        line_sim.set_data(
            sim_pos_km[: frame + 1, 0], sim_pos_km[: frame + 1, 1]
        )
...
def plot_single_trajectory_results(
    ...
    """Plots analysis for a single trajectory dictionary."""
    time_hrs = (sim_results["time"] - sim_results["time"][0]) * TU_TO_HOURS
    ref_pos_km = sim_results["state_ref"][:, :3] * LU_TO_KM
    sim_pos_km = sim_results["state_sim"][:, :3] * LU_TO_KM
    ...
    control_time_hrs = time_hrs[:-1] if len(control_ms2) < len(time_hrs) else time_hrs
    ...
    # PANEL 1: 3D Trajectory Plot (FIXED EQUAL ASPECT)
    ...
    # 3D Equal aspect applied only here
    ...
    if "disturbances" in sim_results and sim_results["disturbances"]:
        for dist in sim_results["disturbances"]:
            ...
            ax3.axvline(
                ...
                alpha=0.8,
                label=(
                    f"ΔV Impulse ({kick_cm_s:.1f} cm/s)"
                    if dist == sim_results["disturbances"][0]
                    else ""
                ),
            )
    ...
    # PANEL 4: LLM Control Acceleration
    ...
    print(f"[*] Saved single trajectory plot to: {save_path}")
    plt.close(fig)
...
def plot_multi_trajectory_comparison(
    ...
    ax_dv = axes[1, 1]
    traj_labels = [f"Traj #{r.get('traj_id', i+1)}" for i, r in enumerate(trajectories)]
    ...
    plt.tight_layout()
    plt.savefig(save_path, bbox_inches="tight")
    print(f"[*] Saved multi-trajectory summary plot to: {save_path}")
    plt.close(fig)
...
if __name__ == "__main__":
    ...
    if os.path.exists(multi_path):
        print(f"[*] Loading multi-trajectory data from '{multi_path}'...")
        loaded = np.load(multi_path, allow_pickle=True)
        trajectories = loaded.tolist() if isinstance(loaded, np.ndarray) else loaded
    elif os.path.exists(single_path):
        print(f"[*] Loading single-trajectory data from '{single_path}'...")
        loaded = np.load(single_path, allow_pickle=True).item()
        trajectories = [loaded]
    else:
        print("[!] No results file found! Please run the simulation script first.")
